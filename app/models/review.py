...
from sqlalchemy import Integer, DateTime, Text, ForeignKey, Index, CheckConstraint, UniqueConstraint
...
class Review(Base):
    ...
    rental_request_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("rental_requests.id"), nullable=True)
    ...
    rental_request = relationship("RentalRequest", back_populates="review", uselist=False, lazy="selectin")

    __table_args__ = (
        CheckConstraint("rating >= 1 AND rating <= 5", name="ck_review_rating"),
        UniqueConstraint("customer_id", "listing_id", "booking_id", name="uq_review_customer_booking"),
        UniqueConstraint("customer_id", "rental_request_id", name="uq_review_customer_rental_request"),
