...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await authApi.login({ email, otp_code: otpCode })
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-slate-950">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 dark:text-slate-400 mt-1">{t('auth.equipmentRentalMgmt')}</p>
        </div>
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-200 dark:border-white/10 p-8">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">{t('auth.signIn')}</h2>
          {error && (
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-4 text-center">
            {t('auth.noAccount')}{' '}
            <Link to="/register" className="text-brand-600 hover:text-brand-700 font-medium">{t('auth.register')}</Link>
          </p>
        </div>
      </div>
    </div>
