...
export default function Sidebar({ user, collapsed = false }: SidebarProps) {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-50 flex flex-col w-64 bg-slate-900 border-r border-white/5">
      {/* Logo */}
      ...
      <div className="px-3 pb-4 shrink-0">
        <Link
          to="/admin/profile"
          className="flex items-center gap-3 px-3 py-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/8 hover:border-white/10 transition-all duration-200 group"
        >
          {user?.avatar_url ? (
            <img
          ...
          {!collapsed && (
            <svg className="w-4 h-4 text-slate-600 group-hover:text-slate-400 transition shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
