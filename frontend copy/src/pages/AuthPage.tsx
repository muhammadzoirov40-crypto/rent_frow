...
export default function AuthPage({ onAuth }: AuthPageProps) {
  ...
  const verifyCode = (code: string) => {
    ...
    setTimeout(() => {
      if (code === MOCK_CODE) {
        setLoading(false)
        if (viewRef.current === 'login-verification') {
          setView('login-success')
        } else {
          setView('register-success')
        }
        setTimeout(() => {
          onAuth('mock-token-' + Date.now(), {
            id: 1,
  ...
  const handleEmailSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false)
      if (viewRef.current === 'login-email') {
    ...
    >
      ...
      <style>{`
        ...
        @keyframes inputFocusPulse {
          0% { box-shadow: 0 0 0 2px rgba(255,213,79,0.4); }
          50% { box-shadow: 0 0 0 4px rgba(255,213,79,0.2); }
          100% { box-shadow: 0 0 0 2px rgba(255,213,79,0.4); }
        }
      `}</style>
            ...
            <div style={{ animation: 'fadeIn 0.4s ease-out' }}>
              ...
              {error && (
                <div className="mb-5 px-4 py-3 rounded-xl text-sm font-medium text-center"
                  style={{ background: 'rgba(239, 68, 68, 0.2)', border: '1px solid rgba(239, 68, 68, 0.3)', color: '#fca5a5', animation: shakeOtp ? 'shake 0.5s ease-in-out' : 'none' }}>
                  {error}
                </div>
              )}
              ...
              <div className="flex justify-center gap-2.5 sm:gap-3 mb-6"
                ...
                {otpDigits.map((digit, i) => (
                  <input
                    ...
                    className="w-11 h-14 sm:w-12 sm:h-16 text-center text-xl sm:text-2xl font-bold rounded-xl outline-none transition-all duration-200 caret-brand-600"
                    style={{
            ...
            <div style={{ animation: 'fadeIn 0.4s ease-out' }}>
              ...
              <div className="mt-6 text-center">
                <p className="text-white/50 text-xs">
                  ...
                  <button onClick={() => { setView(isLogin ? 'register-email' : 'login-email'); resetOtp(); setError('') }}
